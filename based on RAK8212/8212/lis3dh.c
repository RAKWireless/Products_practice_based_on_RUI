...
RUI_I2C_ST lis3dh_st = {0};
#define LIS3DH_ADDR          0x19

uint32_t lis3dh_twi_init(void)
{    
    lis3dh_st.PIN_SDA = 19;
    lis3dh_st.PIN_SCL = 18;
    lis3dh_st.FREQUENCY = RUI_I2C_FREQ_400K;
    rui_i2c_init(&lis3dh_st);
    return NRF_SUCCESS;
}
...
{
    uint32_t err_code;
    err_code = rui_i2c_rw(&lis3dh_st,RUI_IF_WRITE,LIS3DH_ADDR,reg,&data,1);
    return err_code;
}
...
{
    uint32_t err_code;
    err_code = rui_i2c_rw(&lis3dh_st,RUI_IF_READ,LIS3DH_ADDR,reg,p_data,1);
    return err_code;
}
...
int lis3dh_init(void)
{
    ...
    RUI_LOG_PRINTF( "LIS3DH_GetWHO_AM_I = %d", val);
...
{
    LIS3DH_WriteReg(LIS3DH_CTRL_REG1,0x08);
    return 1;
}
/*******************************************************************************
