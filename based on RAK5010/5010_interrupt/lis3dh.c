...
{
    uint32_t err_code;
    err_code = rui_i2c_rw(&st,RUI_IF_WRITE,LIS3DH_ADDR,reg,&data,1);
    return err_code;
}
...
{
    uint32_t err_code;
    err_code = rui_i2c_rw(&st,RUI_IF_READ,LIS3DH_ADDR,reg,p_data,1);
    return err_code;
}
...
int lis3dh_init(void)
{
    ...
    response = LIS3DH_SetInt1Threshold(2);
    if(response==1)
    {
    ...
    response = LIS3DH_SetInt1Duration(5);
    if(response==1)
    {
