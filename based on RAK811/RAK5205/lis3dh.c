...
bool LIS3DH_ReadReg(uint8_t Reg, uint8_t* Data)
{
  ...
  //uint8_t Read_Reg = Reg ;
	ret = rui_i2c_rw(&I2c_1,RUI_IF_READ,0x32,Reg,Data,1);
  // ret = I2cReadBuffer( &I2c, 0x32, Read_Reg, Data, 1 );
  if (ret != 0) {
    RUI_LOG_PRINTF("read lis3dh sensor regester error.\r\n");
  }
...
u8_t LIS3DH_WriteReg(uint8_t WriteAddr, uint8_t Data)
{
  ...
  ret = rui_i2c_rw(&I2c_1,RUI_IF_WRITE,0x32,WriteAddr,(uint8_t *)&Data,1);
  // I2cWriteBuffer( &I2c, 0x32, WriteAddr, (uint8_t *)&Data, 1 );   
  if (ret != 0) {
    RUI_LOG_PRINTF("write lis3dh sensor regester error.\r\n");
  }
...
static uint8_t LIS3DH_INT_STATE = 1;

int16_t ACC_DATA_X = 0;
int16_t ACC_DATA_Y = 0;
int16_t ACC_DATA_Z = 0;
...
void Lis3dh_IntEventClear( void )
{
  u8_t src;
	...
	LIS3DH_GetInt1Src(&src);  //must read this reg
	RUI_LOG_PRINTF("Move Detected INT1 src:0x%02x\r\n", src);
...
void LIS3DH_INT_ENABLE(void){
    LIS3DH_INT_STATE = 1;
    return;
}

void LIS3DH_INT_DISABLE(void){
    LIS3DH_INT_STATE = 0;
    return;
}
...
status_t LIS3DH_GetAccAxesRaw(AxesRaw_t* buff)
{
  ...
  RUI_LOG_PRINTF("ACC X:%dmg Y:%dmg Z:%dmg\r\n",
		buff->AXIS_X, buff->AXIS_Y,buff->AXIS_Z);
...
uint8_t LIS3DH_Init(void)
{
  ...
  if(whoami != 0x33)
  {
     RUI_LOG_PRINTF("LIS3DH is not found!\r\n"); 
     return 0;
  }
  ...
  RUI_LOG_PRINTF("LIS3DH init successed!\r\n");                           
...
/*
rui_inner_function_stru lis3dh_stru=
{
	"lis3dh",
		NULL,
